  ...
  import { useOfficers } from '../hooks/useOfficers'; // Import the hook

  const EditBlotterModal = ({ isOpen, onClose, blotter }) => {
    if (!isOpen || !blotter) return null;

    const queryClient = useQueryClient();
    
    // Fetch officers using the hook
    const { data: officers = [], isLoading: loadingOfficers, isError: officersError } = useOfficers();
      ...
      <Modal onClose={onClose} maxWidth="max-w-lg">
        <div className="relative w-full bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="p-6">
            ...
            {officersError && (
              <div className="mb-3 rounded-md bg-yellow-50 border border-yellow-100 text-yellow-700 px-3 py-2">
                Failed to load officers list
              </div>
            )}
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-gray-200 rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <option value="">
                    {loadingOfficers ? 'Loading officers...' : 'Unassigned'}
                  </option>
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="submit"
                  disabled={isLoading || loadingOfficers}
