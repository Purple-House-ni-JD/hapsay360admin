...
const EditStationModal = ({ isOpen, onClose, selectedStation}) => {
  ...
  const [isEditStationSuccess, setIsEditStationSuccess] = useState(false);
  const [formData, setFormData] = useState({
    id: selectedStation?._id || "",
    name: selectedStation?.name || "",
    address: selectedStation?.address || "",
    phone_number: selectedStation?.contact?.phone_number || "",
    email: selectedStation?.contact?.email || "",
    landline: selectedStation?.contact?.landline || "",
  ...
  const updateStation = async (payload) => {
    const response = await api.put(`police-stations/update/${payload.id}`, payload);
